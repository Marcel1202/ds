...
def coordinate_FRS(task_dict,JOB_ID,addr,encoded_image_list):
    while(True): 
        ...
        flag=unicast_master_slave.main(UPS_ip, UPS_addr, JOB_ID, addr, encoded_image_list)
...
def coordinate_AS(task_dict,JOB_ID,addr,encoded_image_list):
    while(True): 
        min_UPS=list(task_dict.keys())[0]
        min_UPS_task=len(task_dict[list(task_dict.keys())[0]])
        for key,items in task_dict.items():
